...
int16_t decrement_mode(){
	int16_t current_mode = (int16_t)configuration.current_mode;
	int16_t prev_mode_index = (current_mode - 1);
	if (prev_mode_index < 0) {
		prev_mode_index = (int16_t)NUM_LIGHT_MODES - 1;
	}
	set_light_mode_by_index(prev_mode_index);
	return prev_mode_index;
}

const char* get_light_mode_name(int16_t mode_index){
	uint16_t num_modes = sizeof(light_modes) / sizeof(light_mode);
	if (mode_index < 0) {
		return "";
	}
	if ((uint16_t)mode_index >= num_modes) {
		return "";
	}
	return light_modes[mode_index].name;
}
