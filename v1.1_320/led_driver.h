...
#define LED_DATA_1_PIN ( 4 )
#define LED_DATA_2_PIN ( 5 )
...
esp_err_t rmt_new_led_strip_encoder(const led_strip_encoder_config_t *config, rmt_encoder_handle_t *ret_encoder_a, rmt_encoder_handle_t *ret_encoder_b){
    ...
    // WS2812B timing @ 10MHz resolution (0.1µs per tick)
    // bit0: T0H=300ns, T0L=800ns (spec: 350ns/800ns typical)
    // bit1: T1H=700ns, T1L=400ns (spec: 700ns/300ns typical)
    rmt_bytes_encoder_config_t bytes_encoder_config = {
        .bit0 = { 3, 1, 8, 0 },  // 0.3µs high, 0.8µs low = 1.1µs total
        .bit1 = { 7, 1, 4, 0 },  // 0.7µs high, 0.4µs low = 1.1µs total
		.flags = { .msb_first = 1 }
    };
...
void init_rmt_driver() {
	...
	rmt_tx_channel_config_t tx_chan_a_config = {
		...
		.intr_priority = 0,				 // 0 = use default interrupt priority
		.flags = {
			.invert_out   = 0,
			.with_dma     = 0,
			.io_loop_back = 0,
...
IRAM_ATTR void transmit_leds() {
	profile_function([&]() {
		...
		// Both PCBs receive identical data (160 LEDs each)
		if( filesystem_ready == true ){
			rmt_transmit( tx_chan_a, led_encoder_a, raw_led_data, sizeof(raw_led_data), &tx_config );
			rmt_transmit( tx_chan_b, led_encoder_b, raw_led_data, sizeof(raw_led_data), &tx_config );
		}
	}, __func__);
}