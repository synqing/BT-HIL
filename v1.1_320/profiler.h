...
void print_system_info() {
	...
	if (t_now_ms >= next_print_ms) {
		...
		if (!system_info_serial_enabled || hil_csv_serial_export_active) {
			return;
		}
        ...
        if (hil_capture_state) {
            float vu = hil_capture_state->vu_level_capture;
            float novelty_peak = 0.0f;
            for (uint16_t i = 0; i < NOVELTY_HISTORY_LENGTH; i++) {
                float v = hil_capture_state->novelty_curve_normalized_capture[i];
                if (v > novelty_peak) {
                    novelty_peak = v;
                }
            }

            float tempo_peak_mag = 0.0f;
            uint16_t tempo_peak_index = 0;
            for (uint16_t i = 0; i < NUM_TEMPI; i++) {
                float m = hil_capture_state->tempi_magnitude_capture[i];
                if (m > tempo_peak_mag) {
                    tempo_peak_mag = m;
                    tempo_peak_index = i;
                }
            }
            uint16_t tempo_peak_bpm = TEMPO_LOW + tempo_peak_index;

            printf("HIL EXTENDED ----- ENABLED\n");
            printf("HIL VU LEVEL ----- %.3f\n", vu);
            printf("HIL NOVELTY PEAK - %.3f\n", novelty_peak);
            printf("HIL TEMPO PEAK --- %u BPM (mag=%.3f)\n", tempo_peak_bpm, tempo_peak_mag);
        } else {
            printf("HIL EXTENDED ----- BUFFER NOT ALLOCATED\n");
        }
        #endif
