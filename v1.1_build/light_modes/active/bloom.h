#include "goertzel.h" // For chromagram data
...
void draw_bloom() {                                                                                                         
        ...
        // Use chromagram data instead of vu_level for multi-band audio reactivity
        for (uint16_t i = 0; i < NUM_LEDS; i++) {
            float progress_chroma = (float)i / NUM_LEDS;
            novelty_image[i] = clip_float(interpolate(progress_chroma, chromagram, 12));
        }
        ...
        if(configuration.mirror_mode == true){                                                                              
                for(uint16_t i = 0; i < NUM_LEDS>>1; i++){                                                                  
                        ...
                        novelty_pixel *= novelty_pixel; // Apply squaring for punchiness
                        CRGBF color = hsv(                                                                                  
